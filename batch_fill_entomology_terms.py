# -*- coding: utf-8 -*-
"""
수목해충학 용어 설명 일괄 채우기 스크립트
- Gemini API를 사용하여 기본서에서 용어 설명을 검색
- 결과를 JSON 파일로 저장 (중단 시 이어서 작업 가능)

사용법:
  python batch_fill_entomology_terms.py        # 대화형 모드
  python batch_fill_entomology_terms.py --auto # 자동 모드 (확인 없이 실행)
"""
import os
import sys
import django
...
def main():
    print("=== 수목해충학 용어 설명 일괄 채우기 (Gemini API / No-Lock Version) ===")

    target_subject_name = "수목해충학"
    ...
    # 결과 저장용 리스트
    results = []
    output_file = "entomology_explanations.json"
    processed_words = set()
    ...
    for idx, term_data in enumerate(term_list, 1):
        ...
        try:
            # 프롬프트: 수목해충학 관점 강조
            prompt = f"'{term_word}'에 대해 수목해충학 관점에서 자세히 설명해줘. 정의, 특징, 형태, 생활사, 피해 증상, 방제법 등을 포함해서 설명해줘."
    ...
    print("\n다음 단계: apply_entomology_explanations.py 실행하여 DB에 적용")
