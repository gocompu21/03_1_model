...
| 구간 | 상태 |
|---|---|
| 7.1.1 ~ 7.1.7 | 옛 방식 (본문·이미지·문제는 그대로). 선지별 설명만 2026-08-31 채움 |
| 7.1.8 ~ 7.1.42 | **완료** 35개 |
| 7.2.1 ~ 7.2.60 | **완료** 60개 (2026-08-30) |
| 7.3.1 ~ 7.3.4 | **완료** 4개 |
| 7.4.1 ~ 7.4.12 | **완료** 12개 |
| 7.5.1 ~ 7.5.11 | **완료** 11개 (2026-08-31) |

**수목해충학 7장 전체 완료** — 7.1.8 ~ 7.5.11 122개 (7.1.1~7.1.7 옛 방식 7개 제외).
DVD 해충 사진은 7.1.8 ~ 7.5.11 본문 맨 위에 넣었다
(`add_dvd_photo.py`, 확인은 `audit_dvd_photo.py`). 7.1.8 ~ 7.1.18 은 대시보드에서
손으로 올린 PNG 사본(1~2MB)이 있었는데 `swap_dvd_photo.py` 로 같은 규칙의 JPEG
(100~270KB)로 바꿨다. 그 아래 교과서 그림 스캔("그림 7-8 …")은 그대로 두었다.
...
- **완전히 깨끗한 판이 나오면 거기서 멈춘다.** 58개 중 33개가 1번에 통과했다
- **흠이 남았거나 직전보다 나빠졌으면 바로 되돌리지 말고 5번을 채운 뒤 전 판을
  비교해 고른다.** 독립 시행이라 2개만 보고 되돌리면 남은 기회를 버리는 것이다.
  7.5.4 는 2번째가 더 나빴지만 3번째까지 가서 셋 중 최선이 나왔다
- **판마다 파일명(timestamp)이 다르므로 전 판이 남는다.** 고른 판으로
  `/tmp/prep/<id>.json` 의 `image` 를 맞춘다
- 자잘한 오타(알아볼 수 있는 한두 글자)는 통과시키고, 뜻 없는 문장이나 핵심 용어가
  깨진 것, 형태·생활사 오류만 재시도한다
- **이미 저장한 뒤에 잘못을 찾으면 `refix_img.py <id> "지시"`** — 본문의 `<img src>` 만
  갈아 끼운다 (`redo_img.py` 는 저장 전 prep 파일용). 여러 판을 비교할 때는
  `--no-apply` 로 판을 쌓고 `--set <파일명>` 으로 한 번만 바꾼다

### 옛 목차의 선지별 설명 채우기 (2026-08-31)

7.1.1 ~ 7.1.5 는 옛 방식이라 문제에 선지별 설명(`choice_notes`)이 없었다
(7.1.6·7.1.7 은 있었다). 문제·선지·정답·해설은 그대로 두고 빈칸만 채웠다 → 7장 681문제
전부 선지 설명 있음. 다른 과목 연습문제 973개는 아직 전부 없다.

```bash
python /tmp/fill_notes.py 483 484 485          # 선지 하나씩 물어 /tmp/notes/<id>.json 에
python /tmp/renote.py 483 3 4 "덧붙일 지시"     # 한 칸만 다시 (문제번호 3, 선지 4)
python /tmp/fill_notes.py 483 484 485 --go     # 읽어 본 뒤 저장 (태그·$수식$ 은 글자로)
```

- 35문제(175칸) 중 **6칸이 잘못 왔다** — 정답 선지에 ①번 설명이 붙음, 선지 문장을
  그대로 베껴 옴("… (틀림)"), 선지 내용과 무관한 설명(날개 선지에 몸길이·색).
  칸 수는 5개로 맞아 기계는 못 잡는다. **읽어야 보인다.** 목차당 1~2칸꼴
- 다시 물을 때는 지시에 "이 선지는 ~에 대한 것입니다. ~을 근거로 쓰세요"를 넣어야
  같은 오류가 되풀이되지 않는다
- 모델이 `<i>학명</i>`, `$Adoretus\ tenuimaculatus$`, `$9\text{ mm}$` 를 섞어 보낸다.
  다른 설명은 모두 맨글자라 저장 때 글자로 푼다 (첨자 `^ _` 가 있는 진짜 수식만 남김)

### 이미지 재작업 이력 (2026-08-31)

사진 참조(`photoref.py`)와 5판 비교 규칙이 생기기 전에 만든 7.1.8 ~ 7.2.48 의
이미지 80개를 새 규칙으로 다시 만들어 비교했다 (7.1.13·7.1.14·7.2.40 은 이미 새
규칙으로 만든 것이라 제외).

- **교체 70개**, **기존 유지 10개** (7.2.36·37·38·39·41·44·45·46·47·48 — 새 판이
  기존보다 낫지 않았다. 기존 판은 이미 사람이 통과시킨 것이므로 새 판이 **확실히**
  나을 때만 바꾼다)
- 기존 판이 이기는 이유는 대개 새 판의 **생활사 순서**였다 (성충이 부화 앞에 오거나
  부화 라벨이 어긋남). 사진 참조로 형태는 좋아지는데 순환도는 여전히 판마다 흔들린다
- 검증: 7장 129개 목차 모두 인포그래픽 있음, 본문 참조 파일 누락 0
